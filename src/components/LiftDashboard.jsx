...
const LiftDashboard = () => {
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:5000/intialize_data')
            .then(response => {
                setMaxFloor(response.data.max_floor);
                const liftIds = response.data.lift_ids;
                const initialLiftsData = liftIds.map(liftId => ({ id: liftId, alarm: 0, current_floor: 0, door_open: 0 }));
            ...
            .catch(error => console.error('Error fetching initialization data:', error));
    }, []);
    ...
    useEffect(() => {
        ...
        axios.get('http://127.0.0.1:5000/get_all_lifts')
            .then(response => {
    ...
    return (
        <div className="dashboard">
            {liftsData.map((lift) => (
                <LiftLine key={lift.id} liftId={lift.id} liftData={lift} maxFloor={maxFloor} />
            ))}
        </div>
    );    
};
